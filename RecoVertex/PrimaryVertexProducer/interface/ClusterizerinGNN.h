#ifndef ClusterizerinGNN_h
#define ClusterizerinGNN_h

/**\class ClusterizerinGNN
...
class ClusterizerinGNN final : public TrackClusterizerInZ {
public:
...
//ClusterizerinGNN(const edm::ParameterSet& conf,const ONNXRuntime* cache);
ClusterizerinGNN(const edm::ParameterSet& conf, cms::Ort::ONNXRuntime const* onnxRuntime);
...
static void fillPSetDescription(edm::ParameterSetDescription& desc);
  ...
  virtual ~ClusterizerinGNN() override = default;

private:
  const ONNXRuntime* globalCache_;
  float zSep;
  bool verbose_;
  double d0CutOff_;
  double vertexSize_;
